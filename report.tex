...
It's important to take in consideration this 2 metrics to be able to justify wich networck is better than the others.
For example, a networck with a good PSNR result but it's using a lot of flops and param can be less interesting than a networck with less good PSNR result but lower flops and parameters.
...
We can see on this plot the different linear regression calculated on each sample of the test dataset where each they represent one networck.
All the networck are closed to the identity line ($y=x$) which means that the contrast enhancement is closed to the ground truth.
...
% staying with the same resolution
 
\subsubsection{Testing and Measurements} 
% RE not usefull CR more usefull

\paragraph{comparing model with the same computational}

\paragraph{variability accros 10 seeds}
Moreover if we want to compare the results of different models architecture it's important to check the model variability over different seed.
In this case the idea was to used 3 differents seeds for each model and to check the variability over the seed.
Higher is the variability higher is the risk of Knowing the real result of the models because it can change a lot from one seed to another.
...
Two separate plots are produced: one for the Relative Enhancement (RE) and one for the Contrast Ratio (CR).

We can say that for now almost all networck are presenting some good result as we can see with their metrics which are very good.
A lot of networck are having similar result which is normal because they all have a similar base which is the NoNewNet architecture.
...
However, as presented in this architecture the DeepCascade networck are not showing good result mostly because of the DC blocks.
