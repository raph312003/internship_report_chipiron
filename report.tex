...
Based in the 19th arrondissement of Paris, the company brings together around 30 employees from multidisciplinary backgrounds, including Magnetic Resonance Imaging (MRI) physics, hardware engineering, software development, and artificial intelligence.
\vspace{0.5cm}
...
Chipiron chose to focus on MRI for breast imaging applications because, as discussed in the previous section, it represents the most advanced medical imaging modality.
...
In addition, the prevalence of dense breast tissue is higher in the U.S. population than in Europe~\cite{densebreast}, which increases the risk of undetected malignant cancers and therefore strengthens the clinical need for improved breast imaging solutions.
\vspace{0.5cm}
...
\begin{itemize}
    ...
    \item The second clinical phase would involve another group of patients and would focus more specifically on dynamic contrast-enhanced MRI analysis, particularly the evaluation of the wash-in and wash-out phases, which are useful for the diagnosis.
\end{itemize}
...
Chipiron is organized into several departments, the main ones being the hardware team and the software team (Figure~\ref{fig:Employees Chart}).
My hosting department was the software team, within which I was part of the AI team.
The software team is composed of four people: Nina Gidel-Dissier, who is the AI Engineering Manager, and Guillaume Daval-Frérot and Angelo Franciosini, who were both my supervisors.
...
Therefore, the objective of this internship is to perform a feasibility study to evaluate different deep learning models capable of enhancing image quality from accelerated MRI acquisitions.
At the same time, the network must preserve sufficient temporal and spatial information to accurately analyze the contrast enhancement dynamics over time.
\vspace{0.5cm}
...
The following principles guide the way people at Chipiron work.
...
\mbox{}\\
Everyone at Chipiron is expected to act with integrity, honesty, and professionalism.
...
To achieve this, a robust and flexible pipeline was developed, making it easy to evaluate the efficiency of a simple deep learning network.
...
This allowed us to compare different deep learning networks under more realistic conditions for fast low-field MRI acquisition.
\vspace{0.5cm}
...
\begin{itemize}
    \item \textbf{Weeks 1--2:} Literature review (focus on ``Reference-based MRI Reconstruction''). Set up the Python environment.
    \item \textbf{Weeks 3--4:} Data curation. Build the data loader.
    \item \textbf{Weeks 4--8:} Implement a standard reconstruction network (e.g., U-Net) with a functional training pipeline.
...
\begin{itemize}
    \item \textbf{Weeks 8--12:} Refine the architecture (e.g., try different blocks, loss functions, or MRI-specific architectures).
    ...
    \item \textbf{Weeks 12--16:} Work on a more realistic dynamic transition (pipeline modification).
...
\begin{itemize}
    \item \textbf{Weeks 16--20:} Joint optimization: iterate between optimizing undersampling strategies and reconstruction pipelines.
    \item \textbf{Weeks 20--24:} Writing the thesis/report and preparing the final internship presentation. Packaging the code for the team.
\end{itemize}
...
Therefore, the high-quality pre-contrast volume provides valuable structural information that can assist the reconstruction of the accelerated post-contrast images.
Figure~\ref{fig:Training_pipeline_representation} illustrates the proposed training pipeline, where both pre-contrast and accelerated post-contrast volumes are used as inputs to the network.
By adding both volumes to the network, the model is encouraged to focus on the contrast-related changes rather than relearning anatomical information already available in the pre-contrast image. 
This allows the reconstruction process to better preserve and recover the clinically relevant contrast enhancement while benefiting from the high-quality structural reference.
\vspace{0.5cm}
...
Finally, we developed a reliable evaluation framework to ensure that the obtained results were meaningful and reproducible, and ultimately to confirm our first hypothesis.
...
This resizing inevitably changes the spatial resolution of the data, increasing the voxel size from approximately 1.1 × 1.0 × 1.0 mm to 3.3 × 4.0 × 4.0 mm, as shown in Figure~\ref{fig:Resolution_difference}.
...
A shot is a collection of samples acquired consecutively during a sequence (readout).
...
The \texttt{keep\_edges} parameter determines whether the edges of k-space are retained; they carry high-frequency information, but they are not sampled densely enough to make a significant difference.
...
Figure~\ref{fig:cartesian_sampling} illustrates the \texttt{decay\_cartesian} trajectories in 2D and 3D.
\mbox{}\\
...
Figure~\ref{fig:radial_sampling} illustrates the \texttt{stack\_of\_radial} trajectories in 2D and 3D.
\mbox{}\\
...
Figure~\ref{fig:phyllotaxis_radial_mask} illustrates the \texttt{phyllotaxis\_radial} trajectories in 2D.
...
the relevant region of each sample. In many acquisitions, large areas surrounding the object of interest contain only black voxels,
which do not carry any useful information. Keeping these empty regions could negatively influence the model during training, as it may learn to denoise
or reconstruct irrelevant background areas instead of focusing on meaningful structures (Figure~\ref{fig:mask_ground_truth}).
\vspace{0.5cm}
...
areas, which are often the most clinically relevant features, instead of treating all regions of the sample equally (Figure~\ref{fig:segmentation_contrast_mask}).
\vspace{0.5cm}
...
\mbox{}\\
After running the preprocessing script, a dedicated \texttt{LoadingFastMRIBreastDataset3D} class is used to ensure that the dataset is properly organized before being fed into the model.
\vspace{0.5cm}
...
The training script is important because it directly affects how the model learns. In deep learning, the dataset is typically split into a training set and a validation set. 
The training set is used to optimize the model, while the validation set is used during training to monitor performance and tune hyperparameters. 
We need both to ensure that the model does not overfit. 
Overfitting occurs when a model performs very well on the data it was trained on but poorly on new, unseen data. 
Therefore, using separate datasets is essential to assess the model's ability to generalize.
Unlike the validation set, the test set is only used at the end to evaluate the model on unseen data.
...
Gradient descent is an important optimization method used to minimize the cost function, in our case the loss, by progressively adjusting the model parameters, which are mainly the weights and biases.
...
An epoch corresponds to one complete pass of all the samples of our dataset through the model. At the end of each epoch, the average training loss and validation loss are computed.
The goal is to check the model's learning progress by observing how these losses evolve over time, using the Weights \& Biases (W\&B) experiment tracking platform, where training and
...
We created several YAML configuration files to easily change settings. 
Depending on the chosen configuration, we can test different options such as downsampling, undersampling, or the use of segmentation masks. 
There is also a default configuration, which is the simplest setup. 
This allows anyone to run the pipeline directly or modify the YAML files to experiment with different settings. 
This YAML-based setup is very useful because it allows us to test different configurations easily and to identify which one gives the best results.
...
Now that the entire training pipeline was operational, a first neural network could be tested to determine whether its performance was sufficient to validate our first hypothesis.
\vspace{0.5cm}
...
A neural network is composed of several important components, such as convolutional layers, activation functions, normalization layers, and max pooling, which need to be understood before discussing a specific architecture.
\vspace{0.5cm}
...
This slows down training by requiring lower learning rates and careful parameter initialization, and it makes it hard to train models with saturating non-linearities.
...
\begin{itemize}
    ...
    Here, $\mu_B$ represents the mean of the batch and $\sigma_B^2$ the variance of the batch, while $\epsilon$ is a small constant added to avoid division by zero.
...
Finally, max pooling is an operation typically used in CNNs because it reduces the spatial dimensions by sliding a 3D window and taking the maximum value in each region.
...
The first evaluation consists of a qualitative and quantitative analysis of each sample generated by the network, as shown in Figure~\ref{fig:full_sample_qualitative_evaluation}.
...
The second row of Figure~\ref{fig:full_sample_qualitative_evaluation} displays the absolute difference between each image and the ground truth.
...
Finally, the third row of Figure~\ref{fig:full_sample_qualitative_evaluation} presents the signed difference between each image and the ground truth.
...
In Figure~\ref{fig:full_sample_qualitative_evaluation}, the output images are visually very close to the ground truth, both in appearance and in terms of the metrics compared to the input.
...
These metrics are effective for comparing the entire sample, but they do not accurately reflect the quality of the contrast reconstruction.
\vspace{0.5cm}
...
The resulting intensity curves are then plotted for both the model output and the ground truth, as shown in Figure~\ref{fig:dynamic_contrast_intensity}.
\vspace{0.5cm}
...
In Figure~\ref{fig:dynamic_contrast_intensity}, three curves are displayed.
The red curve represents the mean intensity of each segmentation for each network input acquisition of one patient.
The blue curve represents the mean intensity of the network output after contrast reconstruction, while the black curve corresponds to the original (ground truth) contrast mean intensity.
\vspace{0.5cm}

Figure~\ref{fig:full_sample_contrast_dynamic} illustrates the visual evolution of the contrast for each sample.
...
Figure~\ref{fig:Training pipeline full acquisition} illustrates the training pipeline, where the average full acquisition is used as the first input and the different pre- and post-contrast acquisitions are provided as the second input to the network.
...
\subsubsection{Modification of the Training Pipeline} 
...
This averaged image is subsequently used as the full-acquisition image input for reconstructing the pre-contrast and each individual post-contrast acquisition.
\vspace{0.5cm}

However, in the available dataset, only one pre-contrast image and between three and five post-contrast acquisitions are available, rather than ten acquisitions.
...
Instead of directly predicting the post-contrast image, the network predicts the residual, that is, the difference between the pre-contrast and post-contrast acquisitions.
\vspace{0.5cm}
...
This allows the network to keep important voxels while suppressing less relevant ones.
\vspace{0.5cm}
...
The difference between CNN and Multi-Head Attention is that CNN focuses on local information. Each voxel is processed using information from its neighborhood, which allows the network to capture local patterns.
In contrast, Multi-Head Attention works globally.
Each position can interact with all other positions in the image.
This means that each voxel can take into account information from the entire feature map, not just its local surroundings.
\vspace{0.5cm}
...
\begin{itemize}
    ...
    Once the convolution is performed, a fast Fourier transform (FFT) is used to transform the data back into k-space.
    A DC step is then applied in k-space, using the undersampling mask to ensure that only the missing information is updated.
...
In that case, the DC term is very effective, since it enforces agreement with the acquired measurements while allowing the network to estimate the missing coefficients.
\vspace{0.5cm}
...
As a consequence, a strict DC constraint becomes less appropriate, since it forces the reconstruction to remain close to noisy measurements. 
To reduce this limitation, the DC term should ideally be made trainable or adaptive within the network, allowing it to balance fidelity to the measurements and noise suppression. 
...
However, in the context of this internship, where the data follow a different and more realistic noise-corrupted paradigm, the DeepCascade model appears less effective than simpler architectures such as a standard U-Net.
...
As explained in the first phase, the idea is to take one sample from the test dataset and visually compare the input and the output, as shown in Figure~\ref{fig:Noisy full sample qualitative evaluation}.
...
In Figure~\ref{fig:Boxplot quantitative result} we can see the mean PSNR of the output values for each network, and in green the median PSNR of the output values for each network.
...
Figure~\ref{fig:Model variability} illustrates the variability of the network performance across 10 different random seeds using the PSNR, SSIM, and RMSE metrics.
Each boxplot represents the distribution of all the average PSNR values obtained for a given seed.
...
\begin{itemize}
    \item \textbf{Relative Enhancement (RE):} This metric measures the relative increase in signal intensity between the pre-contrast and post-contrast segmentations.
...
Figures~\ref{fig:Quantitative relative enhancement} and \ref{fig:Quantitative contrast ratio} show, for the relative enhancement and the contrast ratio, the different linear regressions computed over all samples of the test dataset, each regression representing one network.
...
Although both metrics lead to similar conclusions regarding the ranking of the models, the contrast ratio shows smaller differences between models than the relative enhancement. 
Since the contrast ratio also incorporates the background intensity, it is less sensitive to variations in the average signal intensity of the contrast region alone.
\vspace{0.5cm}
...
The goal was to evaluate the variability of the slope of each linear regression with respect to the ground truth, and the associated distribution for each linear regression.
\vspace{0.5cm}
...
Figure~\ref{fig:Slope contrast ratio} shows the variability of the slope over 10 seeds for the contrast ratio.
All networks exhibit a similar variability across seeds.
Although UnetAttBlock achieves the highest average slope, the 95\% confidence intervals slightly overlap with those of the other models.
\vspace{0.5cm}

Figure~\ref{fig:slope relative enhancement} shows the variability of the slope across 10 seeds for the RE.
The highest average slope value is obtained by Dense U-Net, with a value of 0.85.
Compared with the CR, the RE metric presents lower slope values and larger variability across seeds, suggesting a weaker agreement between the reconstructed enhancement and the ground truth.
\vspace{0.5cm}

To further evaluate the consistency of the linear relationship, the dispersion around the regression line was analyzed. 
For each seed, the MSE between the estimated CR or RE values and their corresponding linear regression was computed. 
A lower MSE indicates that the data points are closer to the regression line, reflecting a more consistent linear relationship, while a higher MSE corresponds to larger deviations.
\vspace{0.5cm}

Figure~\ref{fig:error contrast ratio} shows the MSE dispersion over 10 seeds for the CR.
All models achieve low MSE values, around 0.01, indicating that the reconstructed CR values remain closely aligned with the regression line.
This suggests that all architectures provide stable CR estimations with limited dispersion.
\vspace{0.5cm}

Figure~\ref{fig:error relative enhancement} presents the MSE variability for the RE metric.
Although all models maintain relatively low dispersion errors, the MSE values are consistently higher than for the CR. 
This indicates that the relative enhancement estimation is less stable and that the reconstructed enhancement curves exhibit larger deviations from the expected linear relationship.


\paragraph{Conclusion on model selection}
\mbox{}\\

Overall, the CR metric appears less challenging, as almost all networks achieve satisfactory performance with limited variability across seeds. 
In comparison, the RE metric leads to lower slope values and higher variability, making the differences between architectures more difficult to interpret and preventing a clear ranking of the models.
\vspace{0.5cm}

Although UnetAttBlock and Dense U-Net show slightly better performance for both RE and CR, the observed differences remain relatively small and are not sufficient to demonstrate a clear advantage over the other architectures.
Therefore, following the same criterion as in the full-sample analysis, the preferred model is the simplest architecture achieving comparable performance. 
In this case, the standard NoNew-Net architecture is selected.
...
As before, we perform two types of quantitative test: the first checks the full-sample enhancement, and the second checks the contrast enhancement only.
Due to time constraints, the model could not be trained on 10 different seeds for this experiment. 
Therefore, the results presented in this section are based on a single seed. 
However, based on the previous model comparison, where the variability across seeds was shown to be limited, this choice is not expected to significantly impact the overall conclusions.

\paragraph{Full-sample enhancement quantitative test}
\mbox{}\\

In Figure~\ref{fig:trajectories quantitative test}, we observe the boxplots for the three different configurations, which are placed next to each other.
...
There is no large difference between the trajectory configurations for the same acquisition time.
...
In the same way as for the full-sample enhancement, we apply the same quantitative analysis used in the network comparison, based on slope and MSE distribution differences across different seeds for the RE and CR.
\vspace{0.5cm}

Figure~\ref{fig:trajectories CR slope} shows the distribution of the slope for each trajectory.
Once again, the undersampling factors of $1/4$ and $1/8$ produce very similar results.
However, for the two highest acceleration factors, the metric degrades significantly, indicating that the network is no longer able to effectively enhance the contrast, which is expected for an acquisition of 2.4~s.
\vspace{0.5cm}

Figure~\ref{fig:trajectories CR error} shows the distribution of the MSE between the linear regression and the measured data.
The MSE values are centered around zero, indicating that there are relatively few outliers.
This suggests that all sampling configurations provide stable CR estimations with limited dispersion.
\vspace{0.5cm}

Figure~\ref{fig:trajectories RE slope} is similar to the previous slope distribution.
Unlike the CR results, the different sampling configurations show more distinct behaviors.
For each acquisition time, the \texttt{phyllotaxis\_radial} configuration consistently achieves higher slope values compared with the two other trajectories.
These results suggest that, for the RE metric, the \texttt{phyllotaxis\_radial} configuration provides more robust performance, with the undersampling factors $1/4$, $1/8$, and even $1/16$ leading to comparable results.
\vspace{0.5cm}

Finally, Figure~\ref{fig:trajectories RE error} shows the residual error associated with the RE slopes.
As in the network comparison, this indicates that the relative enhancement estimation is less stable and that the reconstructed enhancement curves exhibit larger deviations from the expected linear relationship.
...
The three configurations achieve very similar performance, although the \texttt{phyllotaxis\_radial} configuration appears to provide slightly better results, especially for the RE.
\vspace{0.5cm}

Moreover, since the $1/4$ and $1/8$ factors produce very similar results from a physical perspective for each configuration, an undersampling factor of $1/8$ could be selected. 
...
Further experiments with 10 different seeds would still be required to confirm the robustness and reliability of these results.
\vspace{0.5cm}
...
One of the main challenges encountered during this project was handling outliers in the training dataset.
Some samples consistently produced very poor enhancement results, regardless of the acquisition configuration.
\vspace{0.5cm}

These samples were investigated to identify possible causes of their poor performance.
However, this proved difficult, as no single characteristic could explain the failures.
Instead, several factors appeared to contribute to the degradation.
\vspace{0.5cm}

To better understand these relationships, a Principal Component Analysis (PCA) was performed.
The analysis revealed a few weak correlations, notably with the size of the segmented lesion and with small differences in the contrast ratio (CR) between the pre-contrast and post-contrast images.
Nevertheless, these correlations were not strong enough to draw definitive conclusions, and the PCA remained inconclusive.
\vspace{0.5cm}

As a possible extension of this work, the \textit{MAMA-MIA} dataset includes a neural network trained for lesion segmentation.
Evaluating this segmentation model on the enhanced images produced by the proposed network would provide an interesting way to assess whether the enhancement process preserves clinically relevant information.
In particular, it would make it possible to determine whether the segmentation network can still accurately identify the lesion after image enhancement.
\vspace{0.5cm}

Moreover, comparing different architectures and sampling configurations remains a challenging task. 
Drawing reliable conclusions about which model or configuration performs best requires considering several factors beyond the final performance metrics, including computational cost, number of training seeds, and result reproducibility.
\vspace{0.5cm}

For instance, in the final phase of this study, the results were obtained using only a single seed due to time constraints. 
Since neural networks can exhibit variability depending on their architecture and during training, the observed performance may slightly change with additional training repetitions.
Even when using 10 different seeds, it may still be difficult to establish definitive conclusions.
This is why model selection should not rely only on performance metrics but should also consider robustness, reproducibility, and computational efficiency.
...
Several deep learning architectures were compared, and NoNew-Net was identified as the most suitable model for this problem, consistently achieving the best trade-off between reconstruction performance and model complexity.
\vspace{0.5cm}
...
Overall, this work demonstrates that deep learning can significantly reduce MRI acquisition time while maintaining image quality and accurate dynamic contrast enhancement.
...
The quality of this work relies on a systematic evaluation methodology, including the comparison of several deep learning architectures and the analysis of performance variability across multiple training seeds. 
The use of both image reconstruction metrics and contrast-related metrics provided a more comprehensive evaluation of the proposed approaches.
\vspace{0.5cm}

To improve the reliability of the conclusions, experiments were repeated over multiple random seeds. 
This allowed the variability of each model to be quantified and reduced the risk of drawing conclusions from a single training configuration.
\vspace{0.5cm}

However, some limitations remain. 
The evaluation was mainly based on quantitative image metrics and contrast preservation measurements, which do not fully reflect the clinical usefulness of the reconstructed images. 
The final evaluation was conducted using only a single seed, which is insufficient to draw reliable conclusions.
...
\begin{figure}[H]
    ...
    \caption{Neural network helping to reduce acquisition time}
...
\begin{figure}[H]
    ...
    \caption{Full-sample contrast dynamics}
...
\begin{figure}[H]
    ...
    \caption{Training pipeline with the full acquisition as input}
...
\begin{figure}[H]
    ...
    \caption{Boxplot of the quantitative results}
    \label{fig:Boxplot quantitative result}
    \vspace{0.5cm}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Model variability across 10 seeds}
...
\begin{figure}[H]
    \centering
    \vspace{0.5cm}
    \includegraphics[width=0.75\textwidth]{image/Slope model variability for CR across 10 seeds.png}
...
\begin{figure}[H]
    \centering
    \vspace{0.5cm}
    \includegraphics[width=0.75\textwidth]{image/Slope model variability for RE across 10 seeds.png}
...
\begin{figure}[H]
    \centering
    \vspace{0.5cm}
    \includegraphics[width=0.75\textwidth]{image/MSE variability for CR across 10 seeds.png}
...
\begin{figure}[H]
    \centering
    \vspace{0.5cm}
    \includegraphics[width=0.75\textwidth]{image/MSE variability for RE across 10 seeds.png}
...
\begin{figure}[H]
    \centering
    \vspace{0.5cm}
    \includegraphics[width=0.75\textwidth]{image/MSE variability for CR.png}
...
\begin{figure}[H]
    \centering
    \vspace{0.5cm}
    \includegraphics[width=0.75\textwidth]{image/trajectories RE slope.png}
...
\begin{figure}[H]
    \centering
    \vspace{0.5cm}
    \includegraphics[width=0.75\textwidth]{image/MSE variability for RE.png}
