...
The first step consisted of developing a robust and flexible pipeline that could easily be adapted to test different models.
...
The second step involved implementing and comparing several relatively simple neural network architectures. 
...
Once these two initial stages were completed, we moved to a more challenging and realistic setting that better reflected the actual problem. 
Instead of using a clean pre-contrast image, both the pre-contrast and post-contrast images contained noise, making the reconstruction task significantly more difficult.

Finally, after comparing the different models under these more realistic conditions and reaching the limits of their performance through architectural improvements, the focus shifted toward optimizing the overall pipeline and its configuration. 
This included refining preprocessing, training procedures, and hyperparameter settings to further improve the reconstruction quality.

So the goal is to answer the problematic at the end with which trajectories and which model we can have fastest acquisition with the best contrast reconstruction and sample acquisition?
\paragraph{Datasets}
\mbox{}\\
...
\paragraph{ViT}
Have not finish to test it yet.
...
\subsubsection{Full-Sample Qualitative Evaluation}
...
\subsubsection{Contrast Qualitative Evaluation}
...
\subsubsection{Contrast Quantitative Evaluation}
