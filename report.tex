...
The lower field strength reduces the signal-to-noise ratio, leading to noisier images. 
...
Consequently, the signal-to-noise ratio (SNR) is reduced. 
...
Images produced on 1.5~T scanners have improved tremendously since the 1990s, largely thanks to major advances in hardware such as stronger and more sensitive receiver coils, improved gradient systems, and the advent of parallel imaging alongside software improvements which kess vital than hardware.
...
High-field MRI will still be necessary, especially for the most complex clinical cases. 
...
Thanks to advanced AI-based reconstruction methods, the image quality has improved significantly compare to traditional low-field reconstruction approaches.
...
% Company strategy (FDA clearance + getting bought by a bigger company for the manufacturing part?)
...
By accelerating the acquisition it will as explain earlier increase the SNR.
...
The first phase aimed to determine whether deep learning models could reconstruct an enhanced post-contrast image by exploiting both pre-contrast and post-contrast information.
...
Instead of using a clean pre-contrast image,this time both pre-contrast and post-contrast images contained noise, making the reconstruction task significantly more difficult.
...
Since the pre-contrast acquisition is performed only once, it can be acquired with high image quality without strict time constraints. 
...
The Mamamia dataset presents a different challenge, as the samples do not all share the same dimensions.
...
\begin{figure}[H]
    ...
    \caption{Visualization of 3D Cartesian trajectories}
...
\begin{itemize}
    \item The \textbf{post-contrast input}, which is a fully sampled acquisition without undersampling or additional noise. 
    Although it does not contain contrast enhancement, it provides a high-quality reference of the anatomical structures.
    \item The \textbf{pre-contrast input}, which has been undersampled and corrupted with noise to simulate the acquisition of a low-field MRI scanner.
...
\begin{figure}[H]
    \centering
    \vspace{0.5cm}
    \includegraphics[width=0.75\textwidth]{image/Full sample qualitative evaluation.png}
    \caption{Full sample qualitative evaluation}
    \label{fig:Full sample qualitative evaluation}
    \vspace{0.5cm}
\end{figure}
...
\begin{figure}[H]
    \centering
    \vspace{0.5cm}
    \includegraphics[width=0.75\textwidth]{image/Dynamic contrast intensity.png}
    \caption{Full sample qualitative evaluation}
    \label{fig:Full sample qualitative evaluation}
    \vspace{0.5cm}
\end{figure}
...
\begin{figure}[H]
    \centering
    \vspace{0.5cm}
    \includegraphics[width=0.75\textwidth]{image/Full sample contrast dynamic.png}
    \caption{Full sample contrast dynamic}
    \label{fig:Full sample contrast dynamic}
    \vspace{0.5cm}
\end{figure}
...
To better reproduce these conditions, noise was therefore introduced into the pre-contrast acquisition as well. 
...
As explain in the Methodology, we need to add some noise on the pre-contrast so to better simulate this scenario, we propose a different acquisition strategy. 
...
These acquisitions are then averaged to produce the pre-contrast reference image. 
Averaging multiple acquisitions increases the signal-to-noise ratio by exploiting all the available information while preserving the anatomical structures. 
This averaged image is subsequently used as the pre-contrast input for reconstructing each individual post-contrast acquisition.
...
The resulting averaged pre-contrast image is then undersampled using the same sampling mask as the post-contrast acquisitions. 
To account for the longer effective acquisition time, the noise level is reduced by a factor of $\sqrt{10}$. 
This follows the well-known relationship between acquisition time and signal-to-noise ratio in MRI: averaging $N$ independent acquisitions increases the SNR by a factor of $\sqrt{N}$. 
Consequently, using the equivalent of ten acquisitions produces a cleaner and more realistic pre-contrast image while remaining consistent with the physics of MRI acquisition.
...
Although this was sufficient to demonstrate the feasibility of the approach it is not enough to do a fair comparison between different architectures.
...
\begin{figure}[H]
    \centering
    \vspace{0.5cm}
    \includegraphics[width=0.75\textwidth]{image/Noisy full sample qualitative evaluation.png}
    \caption{Noisy full sample qualitative evaluation}
    \label{fig:Noisy full sample qualitative evaluation}
    \vspace{0.5cm}
\end{figure}

We can see the difference between the first phase is the fact that the pre-contrast acquisition is noisy.
...
\begin{figure}[H]
    \centering
    \vspace{0.5cm}
    \includegraphics[width=0.75\textwidth]{image/Boxplot quantitative result.png}
...
Therefore, each model was trained using three different random seeds to evaluate the variability of its results. 
...
As explain above for comparing different model architectures, it is important to evaluate their variability across multiple random seeds, as highlighted in the first phase of the study. 
Initially, each model was tested using three different seeds in order to assess this variability.

However, to improve the reliability of the evaluation, it is preferable to increase the number of seeds beyond three. 
Running experiments over ten seeds would provide a more robust estimation of model stability and allow us to better distinguish between stable and unstable architectures.
...
\begin{figure}[H]
    \centering
    \vspace{0.5cm}
    \includegraphics[width=0.75\textwidth]{image/Model variability.png}
    \caption{Model variability}
    \label{fig:Model variability}
    \vspace{0.5cm}
\end{figure}

This figure illustrates the variability of the network performance across 10 different random seeds using the PSNR metric. 
A similar analysis should also be performed for the RMSE and SSIM metrics.
...
First of all we can see that the networks are all giving an average PSNR of around 34 dB.


\paragraph{Contrast Quantitative Evaluation}

Finaly, the objective of this evaluation is to specifically assess the quality of the contrast enhancement while providing an objective comparison between the different metworks. 
To achieve this, one quantitative metric is used: the Relative Enhancement (RE) 

Relative Enhancement (RE): This metric measures the relative increase in signal intensity between the pre-contrast segmentation and post-contrast segmentation. 
...
\[
x_{\mathrm{RE}} = \frac{x_{\mathrm{post}} - x_{\mathrm{pre}}}{x_{\mathrm{pre}}}
\]


This metrics is computed for every sample and for every network, as well as for the corresponding ground-truth samples.
...
\begin{figure}[H]
    \centering
    \vspace{0.5cm}
    \includegraphics[width=0.75\textwidth]{image/contrast quantitative evaluation.png}
    \caption{contrast quantitative evaluation}
    \label{fig:contrast quantitative evaluation}
    \vspace{0.5cm}
\end{figure}

We can see on this plot the different linear regression calculated on each sample of the test dataset where each they represent one network.
...
Similarly, to compare the performance of the networks for contrast enhancement, as done in the qualitative contrast evaluation, it is necessary to evaluate each model using at least three different random seeds.

%\paragraph{Contrast ratio for Contrast Quantitative Evaluation}
%In our new paradigm, RE is no longer the most appropriate metric to compare models for contrast enhancement. 
%Indeed, evaluating reconstruction quality against the input can be misleading, as the post-contrast images are inherently noisy, which artificially increases the RE.

%That's why we are focusing on auther metrics which is Contrast ratio (CR).
%This metric evaluates the contrast between the segmented lesion and the surrounding background tissue. 
...
Moreover, when comparing different deep learning models, it is important to ensure that all networks are trained under the same GPU memory constraints. 
...
We can say that for now almost all network are presenting some good result as we can see with their metrics which are very good.
A lot of network are having similar result which is normal because they all have a similar base which is the NoNewNet architecture.
It has been explain in the NoNewNet paper that no matter what addition you will add on a NoNewNet it should not increase that much.
However, as presented in this architecture the DeepCascade network are not showing good result mostly because of the DC blocks.
