...
\begin{itemize}
    ...
    This can leads to a non-negligible number of false negatives. 
...
As the company is growing, They aims to make decisions that are responsible, transparent, and respectful of the people Chipiron is working with.. 
The following principles guide the way we work.

\paragraph{accountability}

Chipiron is taking responsibility for thier decisions, actions, and their impact on our colleagues, partners, customers, and society. 
They encourages ownership, continuous learning, and open discussions when mistakes happen so that everyone can improve together. 

\paragraph{transparency}

Chipiron strive to communicate openly and honestly, sharing information whenever appropriate while respecting confidentiality, privacy, and legal obligations. 
At Chipiron, people believe transparency builds trust both inside and outside the company. 

\paragraph{ethical behaviour}

Everyone at Chipiron have to act with integrity, honesty, and professionalism. 
There is no tolerance for fraud, corruption, discrimination, harassment, or conflicts of interest that could compromise our work or our relationships. 
These expectations are supported by Internal Regulations and other internal policies, which define the standards of behaviour expected from everyone working at Chipiron. 

\paragraph{respect for stakeholder interests}

Chipiron decisions consider the interests of the people and organizations connected to their work, including employees, candidates, investors, suppliers, research and clinical partners, customers, and ultimately the patients who may benefit from their technology. 

\paragraph{respect for the rule of law}

Chipiron is committed to complying with the laws and regulations that apply to thier activities in every country where they operate. 
Compliance is a shared responsibility across the company. 

\paragraph{respect for international norms of behaviour}

As an international team, Chipiron strive to create an environment where people from different cultures and backgrounds can collaborate effectively. 
English is the working language, helping ensure that communication is accessible and transparent for everyone. 
They encourage open, respectful communication and value clarity over assumptions, recognizing that cultural differences can influence expectations and communication styles. 
By fostering inclusivity, scientific integrity, responsible employment practices, and respect for different perspectives, Chipiron aim to build a workplace where everyone can contribute and succeed. 

\paragraph{respect for human rights}

Chipiron is committed to treating everyone with dignity, fairness, and respect. They support equal opportunity and strive to maintain a workplace free from discrimination, harassment, and bullying. 
The Internal Regulations (Règlement Intérieur) and related policies establish clear expectations for workplace behaviour, provide channels for reporting inappropriate conduct, and define how concerns are investigated and addressed. 
These rules help ensure that everyone is treated fairly and held accountable for their actions. 
They also expect the suppliers, partners, and other business relationships to share these principles whenever reasonably possible. 
...
We take one full acquisition with pre-contrast and post-contrast and we used the full acquisition as input for our network to enhance the different acquisisition over the time.
...
Now that the preprocessing stage has been explained and realistic low-field MRI data can be generated, including accelerated acquisitions
...
The following sections describe each component in detail and explained how they are combined into a complete training pipeline.
...
\paragraph{Simply artificial neural network introduction}
...
The main script controls the entire training pipeline. It manages the preprocessing script, the LoadingFastMRIBreastDataset3D class as well as all the training steps described above. 
It's also saves the model on Weights \& Biases (wandb) and evaluates it using different metrics that will be explained later. To start the training, I run this main script,
...
Now that the entire training pipeline was operational, a first deep learning network could be experimented to see if the performance is enough suitable to validate our first hypothesis.

As simply explaine earlier neural networks consist of an input layer, multiple hidden layers, and an output layer, whose structure depends on the desired prediction.
...
A neural network is composed of several important components such as CNN, activation function, Normalization and Maxpooling that need to be understood before discussing about a specific architecture.
...
As explained previously, to evaluate whether the neural network is able to enhance post-contrast acquisitions based on accelerated pre-contrast and noisy post-contrast inputs, its performance is assessed according to two main objectives:
\begin{itemize}
...
Finally, we established a comprehensive evaluation strategy to make sure that a simple model network is able to reconstruct both the entire volume and the contrast enhancement.
...
\paragraph{comparing model with the same computational cost}
...
While using 10 seeds the goal is to look at the variability of the slope and the error compare to the ground truth.
...
There are some way of improoving the result without changing the network.
...
The undersampling factors \ref{tab:comparison_acquisition_time} were deliberately pushed to extreme values to clearly identify the point at which the network is no longer able to produce meaningful enhancements.
Moreover, each undersampling factor was evaluated using three different sampling configurations: decay-cartesian, stack-of-radial, and phyllotaxis-radial.
