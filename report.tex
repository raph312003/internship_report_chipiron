...
Chipiron is organized into several departments, the main ones being the hardware team and the software team \ref{fig:Employees Chart}.
...
Therefore, the objective of this internship is to perform a feasibility study to evaluate different deep learning models capable of enhancing image quality from accelerated MRI acquisition.
In the same time, the network must preserve sufficient temporal and spatial information to accurately analyze the contrast enhancement dynamics over time.
\vspace{0.5cm}
...
The figure \ref{fig:Training_pipeline_representation} illustrates the proposed training pipeline, where both pre-contrast and accelerated post-contrast volumes are used as inputs to the network.
...
\begin{itemize}
    ...
    It encodes the spatial information of the image by decomposing it into different frequency components, which can be transformed back into the spatial domain using an inverse Fourier transform (IFT).
...
This figure \ref{fig:cartesian_sampling} illustrates the decay\_cartesian trajectories in 2D and 3D.
\mbox{}\\
...
This figure \ref{fig:radial_sampling} illustrates the stack\_of\_radial trajectories in 2D and 3D.
\mbox{}\\
...
This figure \ref{fig:phyllotaxis_radial_mask} illustrates the phyllotaxis\_radial trajectories in 2D.
...
or reconstruct irrelevant background areas instead of focusing on meaningful structures \ref{fig:mask_ground_truth}.
\vspace{0.5cm}
...
areas, which are often the most clinically relevant features, instead of treating all regions of the sample equally \ref{fig:segmentation_contrast_mask}.
\vspace{0.5cm}
...
The second row of the Figure \ref{fig:full_sample_qualitative_evaluation} displays the absolute difference between each image and the ground truth. 
...
Finally, the third row  \ref{fig:full_sample_qualitative_evaluation}presents the signed difference between each image and the ground truth. 
...
In the Figure \ref{fig:full_sample_qualitative_evaluation}, the output images are visually very close to the ground truth, both in appearance and in terms of the metrics compared to the input. 
...
The Figure \ref{fig:full_sample_contrast_dynamic} illustrates the visual evolution of the contrast for each sample.
...
Figure \ref{fig:Training pipeline full acquisition} illustrates the training pipeline, where the average full acquisition is used as the first input and different pre and post-contrast acquisitions are provided as the second input to the network.
...
This Figure \ref{fig:Model variability} illustrates the variability of the network performance across 10 different random seeds using the PSNR, SSIM and RMSE metrics. 
...
These Figures \ref{fig:Quantitative relative enhancement} and \ref{fig:Quantitative contrast ratio} show, for the relative enhancement and the contrast ratio, the different linear regressions computed over all samples of the test dataset, each regression representing one network.
...
The first Figure \ref{fig:Slope contrast ratio} shows the variability of the slope over 10 seeds for the contrast ratio. 
...
This Figure \ref{fig:slope relative enhancement} shows the variability of the slope across 10 seeds for the RE. 
The highest average slope value is obtained by DenseU-Net, with a value of 0.85. 
...
This Figure \ref{fig:error contrast ratio} shows the MSE dispersion over 10 seeds for the CR. 
...
This Figure \ref{fig:error relative enhancement} presents the MSE variability for RE metric. 
...
Although UNetAttBlock and DenseU-Net show slightly better performance for both RE and CR, the observed differences remain relatively small and are not sufficient to demonstrate a clear advantage over the other architectures. 
...
On this Figure \ref{fig:trajectories quantitative test}, we observe the boxplots for the three different configurations, which are placed next to each other.
...
There is not a big difference between each trajectory config for the same acquisition time.
...
In the same way as for the full-sample contrast enhancement, we apply the same quantitative analysis used in the network comparison, based on slope and MSE distribution differences across different seeds for the RE and CR.
\vspace{0.5cm}

This Figure \ref{fig:trajectories CR slope}, shows the distribution of the slope for each trajectory.
Once again, the undersampling factors of $1/4$ and $1/8$ produce very similar results.
However, for the two highest acceleration factors, the metric degrades significantly, indicating that the network is no longer able to effectively enhance the contrast which is normal with an acquisition of 2.4s.
\vspace{0.5cm}

This Figure \ref{fig:trajectories CR error} shows the distribution of the MSE between the linear regression and the measured data.
The MSE are centered around zero, indicating that there are relatively few outliers.
This suggests that all architectures provide stable CR estimations with limited dispersion.
\vspace{0.5cm}

This Figure \ref{fig:trajectories RE slope} is similar to the previous slope distribution.
Unlike the CR results, the different sampling configurations show more distinct behaviors. 
For each acquisition time, the Phyllotaxis\_radial configuration consistently achieves higher slope values compared with the two other trajectories. 
These results suggest that, for the RE metric, the phyllotaxis\_radial configuration provides more robust performance, with the undersampling factors $1/4$, $1/8$, and even $1/16$ leading to comparable results.
\vspace{0.5cm}

Finally, this Figure \ref{fig:trajectories RE error} shows the residual error associated with the RE slopes. 
This indicates that the relative enhancement estimation is less stable and that the reconstructed enhancement curves exhibit larger deviations from the expected linear relationship exactly like the network comparaison.
...
Since neural networks can exhibit variability depending on their architecture and during the training, the observed performance may slightly change with additional training repetitions. 
