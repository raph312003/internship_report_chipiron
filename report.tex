...
We take one full acquisition with pre-contrast and post-contrast and we used the full acquisition as input for our network to enhance the different acquisition over the time.
...
The model training was tracked online using the Weight \& Biases software. 
...
\begin{itemize}
    ...
    It is basically a binary sampling pattern applied to k-space, where a value of 1 indicates that a k-space location is acquired and a value of 0 indicates that it is omitted.
...
Furthermore, the noise can vary accross k-space. 
...
The timestamps of each acquisitions represent how long it took the MRI device to get the acquisition. 
It is quite important for us because by having the timestamps you can easily separate the acquisitions in the testing phase, so we can see the dynamic of the contrast over time.
...
For each sample, the mean intensity only in the segmented contrast region is computed for every acquisition. 
...
Moreover, it is unconvenient for the patient to first undergo a full pre-contrast acquisition to later do multiple additional post-contrast acquisitions.
...
\paragraph{Variability accross 10 seeds}
...
There are additional ways of improving the result without changing the network.
...
To achieve clinically relevant results, all experiments are performed using the spatial resolution of 4 × 2 × 2 mm targeted by Chipiron. 
...
Each acquisition has a resolution of 4 × 2 × 2 mm with a shape of 40 × 160 × 160.
