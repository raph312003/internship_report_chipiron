...
The projects was divided into three main parts.

The first part consisted of developing a robust and flexible pipeline that could easily be adapted to test different models.

The initial task was to use the clean pre-contrast image as input to reconstruct the post-contrast image. 
This simplified setting allowed us to focus on the model architecture without introducing unnecessary complexity.
The goal was then to implement and compare several relatively simple neural network architectures. 
At the same time, we established a reliable evaluation framework to ensure that the obtained results were meaningful and reproducible.

Once this first part was completed, we moved on to the 2nd part which focused on a more challenging and realistic setting that better reflected the actual problem. 
...
This allowed us to compare the different models under more realistic conditions for fast low-field MRI acquisition and to evaluate the performance of the previously implemented network architectures.
...
The objective of the evaluation is to assess the performance of the different models in order to identify the most suitable architecture and draw meaningful conclusions.

As explained previously, the project has two main objectives:
\begin{itemize}
...
To evaluate these objectives, both qualitative and quantitative analyses are performed. 
...
% Adding a photo 
Once we have done that we can say that our test measurement for the quantitative result are strongly trustworthy. 
In the second part with more accurate input by doing this test we will be able to compare effectively our different networks.
...
We also implemented several network architectures that can be trained and evaluated within the same framework. 
Finally, we established a comprehensive evaluation strategy to compare the different architectures and identify the models that best reconstruct both the entire volume and the contrast enhancement.

In the second phase, we will focus on developing a more accurate representation of the pre-contrast and post-contrast inputs for our fast-acquisition low-field MRI application.

\subsection{2nd part: Transition to Realistic Acquisition Conditions} 

For this second phase the idea is to focus on having a more realistic inputs values and fixing a Deep Learning network.
In this case the resolution and the undersampling configs will not change from the phase one.

\subsubsection{Modification of Training pipeline} 

As explained in the methodology, the first phase of this work focused on developing the training pipeline, evaluating different neural network architectures, and comparing their reconstruction performance.
The objective of the second phase is to make the reconstruction task more realistic by generating input data that better reflects the characteristics of low-field MRI acquisitions.
...
To better simulate this scenario, we propose a different acquisition strategy. 
...
\subsubsection{Architecture models}
For the architecture we have already work on a lot of different network the idea is to use the almost all the same networks and to test them with the new training pipeline.
We will just remove one network which is DeepCascade, even in the first phase his performance where below the others this type of network is not the most usefull for our type of problem.

\subsubsection{Testing and Measurements} 

For Phase 2, the goal is to keep most of the evaluation tests used in Phase 1. 
However, a few minor adjustments will be needed to account for the fact that noise has been added to the pre-contrast images.

\paragraph{Contras ratio for Contrast Quantitative Evaluation}
...
Moreover, when comparing different model architectures, it is important to evaluate their variability across multiple random seeds, as highlighted in the first phase of the study. 
