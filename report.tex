...
Our hosting department was the software team, within whichI was part of the AI team.
...
The parameters associated mostly with the \texttt{decay\_cartesian} trajectories will be systematically explored in the third phase in order to assess their influence and identify the optimal configuration for each acquisition time.
...
For each seed, the mean squared error (MSE) between the estimated CR or RE values and their corresponding linear regression was computed. 
...
In this Figure \ref{fig:trajectories CR slope}, we show the distribution of the slope for each trajectory.
...
These results suggest that, for the RE metric, the phyllotaxis\_radial configuration provides more robust performance for the RE, with the acquisition factors 1/4, 1/8, and even 1/16 leading to comparable results.
...
Moreover, to go even further validate this choice, a radiologist could assess the minimum image quality required to reliably detect a lesion.
