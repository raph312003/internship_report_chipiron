...
\paragraph{Espect for international norms of behaviour}
\mbox{}\\
...
The Internal Regulations and related policies establish clear expectations for workplace behaviour, provide channels for reporting inappropriate conduct, and define how concerns are investigated and addressed. 
...
\paragraph{Using 2 pre-contrast and post-contrast as input}
\mbox{}\\
...
\begin{itemize}
    ...
    \item Mamamia: This dataset contains MRI data from 500 different patients \cite{Mamamia}.
...
\begin{itemize}
    \item \textit{Leviathan}: a server equipped with two 16~GB GPU, mainly used for running experiments and storing the FastMRI Breast and Mamamia datasets.
    \item \textit{Cthulhu}: a more powerful server with two 49~GB GPU, dedicated to training larger and more computationally demanding models.
\end{itemize}
...
The heart is also enhanced by the contrast agent; however, it is not included in the segmentation mask. 
In the Mamamia Dataset, \cite{Mamamia} the segmentation masks were initially generated automatically using a segmentation network and subsequently reviewed and corrected by multiple experts, resulting in highly accurate annotations.
...
The timestamps of each acquisition represent how long it took the MRI device to get the acquisition. 
...
it must be carefully tuned. I chose a lr of 0.0001 which slow down a bit the training but allows finding quite well the convergence of the cost function.
...
It also saves the model on Weights \& Biases (wandb) and evaluates it using different metrics that will be explained later. To start the training, I run this main script,
...
These filters, which can have dimensions such as (3x3x3) or (5x5x5) in the case of three-dimensional images, the weights of the filters are optimized during training to detect different types of features within the images.
...
Batch Normalization significantly improve training stability by reducing internal covariate shift and accelerated convergence.
...
In the other way Instance Normalization, it doesn’t directly improve training speed or stability but it allows to train models with large inputs (e.g., medical images) with small batch sizes.
...
\paragraph{NoNew-Net}
\mbox{}\\
The first Architecture I decided to investigat to validate the first hypothesis was the NoNew-Net \cite{NoNewNet}, it's based on a U-Net Architecture.
...
The main argument of the NoNew-Net is that the architecture outperforms a more complicated one without unnecessary complicated layers \cite{NoNewNet}
...
In our case, the NoNew-Net offers the best compromise between reconstruction quality, architectural simplicity, and computational cost, making it the most suitable model for the fully sampled enhancement task.
...
\begin{itemize}
    ...
    \[
    {\mathrm{RE}} = \frac{mean_{\mathrm{ROI}_{\mathrm{post}}} - mean_{\mathrm{ROI}_{\mathrm{pre}}}} {mean_{\mathrm{ROI}_{\mathrm{pre}}}}
    \]
    Where ROI (Region-of-interest) indicates the tumor segmentation region.
    ...
    \[
    {\mathrm{CR}} = \frac{mean_{\mathrm{ROI}} - mean_{\mathrm{background}}} {mean_{\mathrm{background}}}
    \]
...
A linear regression is then created for each model depending on the result of all samples for the network. 
...
This plot shows the variability of slope over 10 seeds for the contrast ratio, we can see that the best slope value is 0.86 and it corresponds to UnetAttBlock.
...
This plot shows the variability of slope over 10 seeds for the Relative enhancement, we can see that the best slope valuz is 0.86 and it corresponds also to UnetAttBlock.
...
For contrast enhancement, it appears that almost all network are achieving acceptable results.
...
\subsubsection{Conclusion} 
In this second phase, we explored a new paradigm by using the average of the full acquisition for enhancing fast pre-contrast and post-contrast acquisitions. 
We also implemented and compared different network architectures in order to evaluate which one was the most efficient for dynamic contrast enhancing.

Overall, most architectures produced good results. 
Therefore, we selected the simplest architecture, NoNewNet, as it offered a good trade-off between performance and simplicity.
...
As we did for the first network, we will use boxplot and doing the training on 10 different seeds to check in the same time the variability of the training.
...
In this plot we can see logically than more the acquisition is being reduced more the metrics are getting worst which totally logical.
...
The goal of this final phase was to evaluate three different acquisition configurations and determine how much the acquisition time could be reduced depending on the config while maintaining sufficient image quality and contrast enhancement.
The three configurations achieve very similar performance, although the decay-cartesian configuration appears to provide slightly better results, especially for highly accelerated acquisitions.
...
The first phase focused on validating whether deep learning models could reconstruct enhanced post-contrast images by using fully-sampled post-contrast and fast post-contrast acquisition. 
A complete and flexible pipeline was developed to generate fast acquisitions and test a simple architecture. 
The results showed that deep learning models were able to improve accelerated post-contrast with both pre-contrast and post-contrast acquisitions while preserving the contrast enhancement information.

The second phase introduced a more realistic acquisition scenario closer to fast low-field MRI conditions. 
Instead of relying on fully sampled acquisitions, the models were trained using a full acquisitions with both pre-contrast and post-contrast information to reconstruct the contrast evolution over time. 
...
Overall, this work demonstrates that deep learning can significantly reduce MRI acquisition time while maintaining image quality and contrast dynamic enhancement.
...
In a subsequent clinical study, the focus will shift toward evaluating the ability of the method to accurately capture contrast dynamics, particularly the wash-in and wash-out phases, which are essential to perform a diagnostic.
